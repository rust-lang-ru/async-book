...
1. Если `T: Unpin` (что по умолчанию), то `Pin<'a, T>` полностью эквивалентен `&'a mut T`. Другими словами: "`Unpin`" означает, что этот тип можно перемещать, даже если он закреплён, поэтому "`Pin`" не повлияет на такой тип.

2. Преобразование `&mut T` в закреплённый T требует unsafe, если `T: !Unpin`.
